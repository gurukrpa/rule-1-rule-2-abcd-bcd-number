-- ==========================================
-- SAFE DATABASE MIGRATION FOR TRUE INDEPENDENCE
-- Handles orphaned data and missing users
...
-- Step 6: Clean orphaned data and migrate safely
DO $$
DECLARE
...
BEGIN
    ...
    IF table_exists THEN
        -- Check for orphaned data (user_dates records with no corresponding user)
        ...
        FROM user_dates ud
        INNER JOIN users u ON ud.user_id = u.id  -- Only include records where user exists
...
SELECT 
  ...
  ud.user_id,
  ud.dates,
  'Consider cleaning up manually' as action
FROM user_dates ud
LEFT JOIN users u ON ud.user_id = u.id
WHERE u.id IS NULL;
