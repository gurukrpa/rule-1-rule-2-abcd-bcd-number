// src/components/Rule1PageOptimized.jsx
...
function Rule1PageOptimized() {
  ...
  // Batch click processing for optimal performance
  const processBatchedClicks = useCallback(() => {
    if (pendingClicks.size === 0) return;

    const clicksToProcess = Array.from(pendingClicks);
    setPendingClicks(new Set());

    // Process all pending clicks in one batch
    optimizedRule1Service.processBatchClicks(selectedUser, clicksToProcess)
      .then(() => {
        console.log(`✅ [Rule1Optimized] Processed ${clicksToProcess.length} clicks`);
      })
      .catch(error => {
        console.error('❌ [Rule1Optimized] Batch click error:', error);
        // Re-add failed clicks for retry
        setPendingClicks(prev => new Set([...prev, ...clicksToProcess]));
      });
  }, [selectedUser, pendingClicks]);
  ...
  useEffect(() => {
    if (pendingClicks.size > 0) {
      ...
      batchUpdateTimeoutRef.current = setTimeout(processBatchedClicks, 100);
    }
  ...
  }, [pendingClicks, processBatchedClicks]);
  ...
  const handleUserChange = useCallback((newUserId) => {
    if (newUserId !== selectedUser) {
      ...
      navigate(`/rule1-optimized/${newUserId}`);
    }
  }, [selectedUser, navigate]);

  // Date navigation with preloading
  const handleDateChange = useCallback((direction) => {
    const currentIndex = datesList.findIndex(date => date === selectedDate);
    let newIndex;
    
    if (direction === 'prev' && currentIndex > 0) {
      newIndex = currentIndex - 1;
    } else if (direction === 'next' && currentIndex < datesList.length - 1) {
      newIndex = currentIndex + 1;
    } else {
      return;
    }
    ...
    if (newIndex > 0) {
      optimizedRule1Service.preloadDateData(selectedUser, datesList[newIndex - 1]);
    }
    if (newIndex < datesList.length - 1) {
      optimizedRule1Service.preloadDateData(selectedUser, datesList[newIndex + 1]);
    }
  }, [datesList, selectedDate, selectedUser]);
