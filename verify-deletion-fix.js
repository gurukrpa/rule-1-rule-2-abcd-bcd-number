 ...
 * 1. The deletion functionality is working properly 
 * 2. The auto-upload status bug has been resolved
 * 3. Deleted dates don't show false positive upload status when re-added
 */
...
async function cleanupBeforeTest() {
  ...
  const tables = [
    ...
    { name: 'hour_entries', userCol: 'user_id', dateCol: 'date_key' },
  ...
  for (const table of tables) {
    try {
      if (table.dateCol) {
        ...
        // user_dates table - just clear the dates array
        const { error } = await supabase
          .from(table.name)
          .delete()
          .eq(table.userCol, TEST_USER_ID);
...
async function testComprehensiveDeletion() {
  ...
  // Import and use the DataService deletion function
...
async function simulateReAddingDate() {
  ...
  // Add the date back to user_dates (but no data)
  const { error } = await supabase
    .from('user_dates')
    .upsert([{
      user_id: TEST_USER_ID,
...
async function runVerificationTest() {
  try {
    ...
    // Step 4: Check state after creation (should have data)
    const afterCreation = await testDataExistence('After creating test data:');
    
    // Step 5: Test comprehensive deletion
    ...
    // Step 6: Check state after deletion (should be empty)
    const afterDeletion = await testDataExistence('After comprehensive deletion:');
    
    // Step 7: Simulate re-adding the date
    ...
    // Step 8: Check final state (date should exist, but no upload data)
